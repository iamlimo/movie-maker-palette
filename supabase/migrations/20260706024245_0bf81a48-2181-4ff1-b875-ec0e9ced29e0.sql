CREATE OR REPLACE FUNCTION public.expire_canonical_rental_access(p_skew_minutes integer DEFAULT 0)
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET status = 'failed'::public.rental_intent_status,
      ...
      metadata = COALESCE(metadata, '{}'::jsonb) || jsonb_build_object(
        'expired_by', 'expire_canonical_rental_access',
        'expired_at', now()
      )
  WHERE status = 'paid'::public.rental_intent_status
    AND revoked_at IS NULL
    AND expires_at <= (now() - make_interval(mins => GREATEST(COALESCE(p_skew_minutes, 0), 0)));
...
GRANT EXECUTE ON FUNCTION public.expire_canonical_rental_access(integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.expire_canonical_rental_access(integer) TO service_role;
